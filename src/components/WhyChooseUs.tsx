...
const WhyChooseUs = () => {
  return (
    <section data-reveal="fade-up" className="py-24 bg-slate-50 relative overflow-hidden">
      ...
      <div className="container-custom relative z-10">
        {/* Section Header */}
        <div data-reveal="fade-up" className="text-center max-w-3xl mx-auto mb-12 md:mb-16 space-y-4">
          ...
          <p className="text-slate-600 text-xl font-medium leading-relaxed text-balance">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => (
            <div
              ...
              className={`group p-8 rounded-[2rem] bg-white border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 relative overflow-hidden`}
            >
              <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-slate-50 to-slate-100 rounded-bl-[60px] -mr-8 -mt-8 transition-transform group-hover:scale-150 duration-500 z-0`} />

              <div className="relative z-10 flex flex-col items-start h-full">
                <div className={`w-14 h-14 rounded-2xl ${benefit.bg} flex items-center justify-center mb-6 transition-all duration-300 shadow-sm group-hover:shadow-md ring-1 ring-slate-100 group-hover:ring-transparent`}>
                  <benefit.icon className={`w-7 h-7 ${benefit.color} group-hover:text-white transition-colors duration-300`} />
                </div>

                <h4 className="font-bold text-slate-900 text-xl mb-3 group-hover:text-primary transition-colors">
                  {benefit.title}
                </h4>
